...
export const useAddTodo = (options = {}) => {
  ...
  return useMutation({
    mutationFn: (newTodo) =>
      postToAPI({ url: apiBatch.postTodos, data: { todo: newTodo } }),
...
export const useDeleteTodo = (options = {}) => {
  ...
  return useMutation({
    mutationFn: (todo) =>
      deleteToAPI({ url: apiBatch.deleteTodos, data: { id: todo.id } }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['todos'] }),
    ...options,
  })
}
...
export const useToggleTodo = (options = {}) => {
  ...
  return useMutation({
    mutationFn: (todo) =>
      patchToAPI({ url: apiBatch.postTodos, data: { id: todo.id } }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['todos'] }),
    ...options,
  })
}
